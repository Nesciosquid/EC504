...
import EC504.highScores.highScore;
import EC504.highScores.highScore.hsComp;
import EC504.samegame.Brain.Brain;
...
class sg extends GUI implements ActionListener,ItemListener {
	...
	// constants
	static public final int defaultWidth=100, defaultHeight=100; // sizes in terms of numbers of circles
	...
	/* Modified to make the game play much faster for larger datasets! */
	static public final int displayTime = 0; // number of milliseconds to display (highlight) a move before actually making it (original: 1000)
	/* -------------- */
	...
	JLabel regionPoints = new JLabel("0"); // keeps track of the number of selected points
	JLabel totalPoints = new JLabel("0");  // keeps track of the total number of points so far
	...
	private JRadioButtonMenuItem noPlayerMenuItem,greedyPlayerMenuItem,simplePlayerMenuItem, lazyPlayerMenuItem, // various automatic player options
	 ...
	 */
	final public int score(int level) {
		return 1;
	}
	...
	final private void addNeighbor(int ii, int jj, int xx, int yy) {
		if ((0<=xx && xx<width) &&
				(0<=yy && yy<height))
	...
	void setupGUI() {
		...
		// set the layout style for the whole applet
		...
		greedyPlayerMenuItem = new JRadioButtonMenuItem("Greedy player");
	 ...
	 */
	private final boolean sameColor(CircleColor theColor, int xx, int yy) {
	 ...
	 */
	private final void moveCircle(SelfAwareCircle orig, SelfAwareCircle dest) {
	...
	final boolean shiftCircles() {
		...
		if (hs.newRecordQ(score)) { // i.e. a new record
			JOptionPane.showMessageDialog(null, "Game Over - You got a new high score of "+score+" points!\n"+
					"Your weighted score for this sized board was "+ hsComp.weightedScore( new highScore.HSdata("",width,height,score))+".");
	...
	private void startBrain(Brain theBrain) {
		...
		highScoreName = theBrain.name(); // the computer gets credit for any subsequent high score
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (e.getStateChange() == ItemEvent.SELECTED) {
			if (source == noPlayerMenuItem)
				...
				startBrain(new EC504.samegame.Brain.simpleBrain(this));
			else if (source == lazyPlayerMenuItem)
				startBrain(new EC504.samegame.Brain.lazyBrain(this));
			else if (source == greedyPlayerMenuItem)
				startBrain(new EC504.samegame.Brain.GreedyBrain(this));
			else if (source == smarterPlayerMenuItem){
				startBrain(new EC504.samegame.Brain.SmarterBrain_Aaron(this));
			}
/*
...
*/
			...
			// deselected
			if (theBrain!=null)
				theBrain.done(); // will ask the Brain to stop thinking
		}
	}

	/**
	 * Designed to allow the applet to run as a standalone application
	 * @see http://www.wutka.com/hackingjava/ch13.htm (this is based on listing 13.1)
	 */
	public static void main(String args[]) {
		IamApplet = false; // i.e. this is being invoked as an application
		
		JApplet myApplet = new sg();
		JFrame appletFrame = new JFrame("Ari's samegame");
		appletFrame.setDefaultCloseOperation (JFrame.EXIT_ON_CLOSE);
		appletFrame.setSize(defaultWindowWidth, defaultWindowHeight);
