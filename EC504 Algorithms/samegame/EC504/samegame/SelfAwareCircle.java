...
class SelfAwareCircle extends Component implements MouseListener, SelfAwareListener {
	...
	private ArrayList<SelfAwareListener> _listeners = new ArrayList<SelfAwareListener>(); // who is listening to events fired by this circle
	private static int id=0;
	private int myid; // the circle's ID
	private Visibility state = Visibility.plain;	// the current state of the circle
	private sg gui; // a link to the GUI that created this circle

	// concurrency variables
	private static AtomicInteger regionLength = new AtomicInteger(0);
	...
	public static enum Visibility{
		...
		public void mouseReleased(MouseEvent e) {
			...
			gui.totalPoints.setText("" +
					(Integer.parseInt(gui.totalPoints.getText()) + gui.score(SelfAwareCircle.getRegionLength())));
		...
		private synchronized void _fireButtonRolloverEvent(SelfAwareCircle.Visibility state) {
			...
			Iterator<SelfAwareListener> lers = _listeners.iterator();
			while (lers.hasNext()) {
				SelfAwareCircle temp = (SelfAwareCircle) lers.next();
				( (SelfAwareListener) temp ).rollingOver(bre);
			}
		}
